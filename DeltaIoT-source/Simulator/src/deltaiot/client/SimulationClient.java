...
public class SimulationClient implements Probe, Effector {
	...
	public SimulationClient(){
		this.simulator = deltaiot.DeltaIoTSimulator.createSimulatorForDeltaIoT(DeltaIoTEmulatorMain.selectedUncertainty);
	}
...
//	}
	...
	//for manage Uncertain Local congestion in mote
	@Override
	public int getQueueFilling(int moteId) {
		return getMote(moteId).getPacketQueue().size();
		
	}
	public int getSizeOfQueue(int moteId) {
		return getMote(moteId).getQueueSize();
	}
}
